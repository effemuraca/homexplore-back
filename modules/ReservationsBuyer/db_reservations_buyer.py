...
class ReservationsBuyerDB:
    ...
    def delete_reservation_by_user_and_property(self, user_id: int, property_id: int) -> bool:
        ...
        if not raw_data:
            return False
        data = json.loads(raw_data)
        new_data = [item for item in data if item.get("property_id") != property_id]
        if len(new_data) == len(data):
            return False
        result = redis_client.set(f"buyer_id:{user_id}:reservations", json.dumps(new_data))
        return bool(result)
    
    def update_reservation_buyer(self, user_id: int, reservation_id: int, reservation: ReservationB) -> bool:
        if not user_id or not reservation_id or not reservation:
            return False
        redis_client = get_redis_client()
        raw_data = redis_client.get(f"buyer_id:{user_id}:reservations")
        if not raw_data:
